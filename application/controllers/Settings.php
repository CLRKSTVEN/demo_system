	...
	public function Subjects()
	{
		...
		if ($this->input->post('save')) {
			$data = [
				...
				'sem'          => $this->input->post('sem') ?? '',
				'strand'       => $this->input->post('strand') ?? '',
				'subCategory'  => $this->input->post('subCategory') ?: 'None',
	...
	public function updatesubjects()
{
    ...
    $data = [
		...
		'subCategory' => $this->input->post('subCategory', true) ?: 'None',
    ];
	...
	// Return YearLevels available for current SY (exclude 0/blank)
public function getYearLevels()
{
    $sy = $this->session->userdata('sy');
    $this->load->model('SettingsModel');
    $rows = $this->SettingsModel->getDistinctYearLevelsForSY($sy);
    echo json_encode($rows); // [{YearLevel:"Grade 07"}, ...]
}
