...
class PersonnelModel extends CI_Model 
{
    ...
    {
        $this->db->where('fileID', $param);
        unlink("upload/" . $path . "/" . $attach);
        $this->db->delete($table, array('fileID' => $param));
    }

	function delete_announcement($param, $attach, $path, $table)
    {
        $this->db->where('aID', $param);
        unlink("upload/" . $path . "/" . $attach);
        $this->db->delete($table, array('aID' => $param));
    }
