...
class Common extends CI_Model
{
  ...
  public function two_join_one_cond($t1, $t2, $select, $joinby, $where, $gb, $ob, $obval)
{
    $this->db->select($select);
    $this->db->from($t1 . ' as a');
    $this->db->join($t2 . ' as b', $joinby, 'left');

    // $where can be array or string
    if (is_array($where)) {
        $this->db->where($where);
    } else {
        // backward compatible: single string column name is not supported here anymore
        // but you can keep both styles if you want:
    ...
    public function two_join_six_cond_count($t1, $t2, $select, $joinby, $col, $val, $col2, $val2, $col3, $val3, $col4, $val4, $col5, $val5, $col6, $val6)
    {
        $this->db->select($select);
        $this->db->from($t1 . ' as a');
        $this->db->join($t2 . ' as b', $joinby, 'left');
    ...
    public function three_cond_gb($table, $field1, $value1, $field2, $value2, $field3, $value3, $groupBy, $orderBy, $order = 'ASC')
    {
        $this->db->where($field1, $value1);
        $this->db->where($field2, $value2);
    ...
    public function two_cond_gb($table, $field1, $value1, $field2, $value2, $groupBy, $orderBy, $order = 'ASC')
    {
        $this->db->where($field1, $value1);
        $this->db->where($field2, $value2);
        $this->db->group_by($groupBy);
    ...
    public function get_subject_enrollees($sy, $section, $subjectcode, $strand = null)
    {
        $this->db->select('a.YearLevel, a.StudentNumber, b.StudentNumber AS studeStudentNumber,a.strand, a.SY, a.Sem, b.FirstName, b.MiddleName, b.LastName, b.Sex');
        $this->db->from('registration a');
        $this->db->join('studeprofile b', 'a.StudentNumber = b.StudentNumber');
        $this->db->where('a.SY', $sy);
        $this->db->where('a.Section', $section);
        $this->db->where('a.SubjectCode', $subjectcode);

        // Only add the strand filter if it is provided
        if (!empty($strand)) {
            $this->db->where('a.strand', $strand);
        }

        $this->db->order_by("CASE WHEN b.Sex = 'Male' THEN 0 WHEN b.Sex = 'Female' THEN 1 ELSE 2 END", 'ASC');
        $this->db->order_by('b.LastName', 'ASC');

        return $this->db->get()->result();
    }

  public function get_subject_grade_records($sy, $section, $subjectcode, $strand = null)
{
    $this->db->select([
        'a.gradeID',
        'a.YearLevel',
        'a.StudentNumber',
        'b.StudentNumber AS studeStudentNumber',
        'a.strand',
        'a.SY',
        'a.Sem',
        'b.FirstName','b.MiddleName','b.LastName','b.Sex',
        'a.Instructor',
        'a.SubjectCode',
        'a.Description',
        'a.Section',
        ...
        // ✅ letter grades
        'a.l_p','a.l_m','a.l_pf','a.l_f','a.l_average',

        'a.firstStat','a.secondStat','a.thirdStat','a.fourthStat'
    ]);
    $this->db->from('grades a');
    $this->db->join('studeprofile b', 'a.StudentNumber = b.StudentNumber', 'inner');
    ...
    // In application/models/Common.php

// Version 1: sort by Sex then LastName
public function cons($yearLevel, $section, $sy)
{
    $this->db->select("
        r.StudentNumber,
        sp.LastName, sp.FirstName, sp.MiddleName, sp.Sex,
        ROUND(AVG(g.Average), 2) AS Average
    ");
    $this->db->from('registration r');
    $this->db->join('studeprofile sp', 'sp.StudentNumber = r.StudentNumber', 'left');
    $this->db->join('grades g', "g.StudentNumber = r.StudentNumber AND g.YearLevel = r.YearLevel AND g.SY = r.SY", 'left');
    $this->db->where('r.SY', $sy);
    $this->db->where('r.YearLevel', $yearLevel);
    $this->db->where('r.Section', $section);
    $this->db->group_by(['r.StudentNumber', 'sp.LastName', 'sp.FirstName', 'sp.MiddleName', 'sp.Sex']);
    $this->db->order_by('sp.Sex', 'ASC');
    $this->db->order_by('sp.LastName', 'ASC');
    $this->db->order_by('sp.FirstName', 'ASC');
    return $this->db->get()->result();
}

// Version 2: sort by Average (DESC), then LastName
public function consv2($yearLevel, $section, $sy)
{
    $this->db->select("
        r.StudentNumber,
        sp.LastName, sp.FirstName, sp.MiddleName, sp.Sex,
...
// Version 3: sort by LastName (ASC)
public function consv3($yearLevel, $section, $sy)
{
    $this->db->select("
        r.StudentNumber,
        sp.LastName, sp.FirstName, sp.MiddleName, sp.Sex,
