...
$period_field  = null;
switch ($grading_code) {
    case 'PGrade':      $period_field = 'lock_prelim';  break;
    case 'MGrade':      $period_field = 'lock_midterm'; break;
    case 'PFinalGrade': $period_field = 'lock_prefinal';break;
    case 'FGrade':      $period_field = 'lock_final';   break;
}
...
if ($period_field && !empty($sub)) {
    foreach ($sub as $r) {
        ...
        if ($locks && !empty($locks->{$period_field})) { $anyLockedThisPeriod = true; break; }
    }
}
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <style>
                                    ...
                                    .subject-th { min-width: 140px; }
                                    .d-none { display:none !important; }
.subject-th .d-flex.flex-column { gap: 6px; }
...
@media (max-width: 640px){
  .lock-pills{ flex-wrap: wrap; white-space: normal; }
}
...
.lock-pill.open{   background: #e9f7ef; } 
.lock-pill.locked{ background: #fdecea; } 
...
.lock-pill:hover { filter: brightness(0.95); }
.lock-mini:hover { box-shadow: 0 0 0 1px rgba(0,0,0,.15) inset; }

.subject-th .lock-pills{ margin-top: 2px; }
...
function queuePageReload(delayMs = 0) {
    ...
    if (delayMs <= 0) {
        ...
        return requestAnimationFrame(function () { window.location.reload(); });
    }
    _reloadTimer = setTimeout(function () { window.location.reload(); }, delayMs);
}

  // ---------- Per-subject locking handlers ----------
  function updateSubjectBadges(sid, locks, isShs) {
      const map = [
          { id: '#lk-' + sid + '-prelim',   val: locks.lock_prelim,  title: '1st Grading: ' },
          { id: '#lk-' + sid + '-midterm',  val: locks.lock_midterm, title: '2nd Grading: ' }
      ];
      if (!isShs) {
          map.push(
              { id: '#lk-' + sid + '-prefinal', val: locks.lock_prefinal, title: '3rd Grading: ' },
              { id: '#lk-' + sid + '-final',    val: locks.lock_final,    title: '4th Grading: ' }
          );
      }
      map.forEach(m => {
          const $b = $(m.id);
  ...
  function recomputeSemesterBadge() {
    ...
    var anyLocked = $('.lock-pill[data-period="'+ CURRENT_PERIOD +'"]').toArray()
      .some(function(el){ return el.classList.contains('locked'); });
...
$(function () {
    ...
    if (typeof $.fn.dataTable === 'undefined') { console.error('DataTables core not loaded.'); return; }
    if (typeof $.fn.dataTable.Buttons === 'undefined') { console.error('DataTables Buttons not loaded.'); return; }
    if (typeof JSZip === 'undefined') { console.warn('JSZip not loaded (Excel export will be disabled).'); }
    if (typeof pdfMake === 'undefined') { console.warn('pdfMake not loaded (PDF export will be disabled).'); }
    ...
    window.dtExport = $clone.DataTable({
        ...
        buttons: (function(){
            var arr = [];
            arr.push({ extend: 'csvHtml5',   title: exportTitle, filename: exportFilename });
            if (typeof JSZip !== 'undefined') {
                arr.push({ extend: 'excelHtml5', title: exportTitle, filename: exportFilename });
            }
            if (typeof pdfMake !== 'undefined') {
                arr.push({ extend: 'pdfHtml5',   title: exportTitle, filename: exportFilename,
                           orientation: 'landscape', pageSize: 'A4',
                           customize: function (doc) {
                               if (doc.styles && doc.styles.tableHeader) { doc.styles.tableHeader.alignment = 'left'; }
                               doc.defaultStyle = doc.defaultStyle || {}; doc.defaultStyle.fontSize = 9;
                           }
                });
            }
            return arr;
    ...
    $('#btnExportExcel').on('click', function(e){
        ...
        if (b.any()) b.trigger(); else alert('Excel export unavailable. (JSZip not loaded?)');
    });
    $('#btnExportPDF').on('click', function(e){
        e.preventDefault();
        var b = dtExport.button('.buttons-pdf');
        if (b.any()) b.trigger(); else alert('PDF export unavailable. (pdfMake not loaded?)');
    });
    $('#btnExportCSV').on-click = null;
    $('#btnExportCSV').on('click', function(e){
        e.preventDefault();
        var b = dtExport.button('.buttons-csv');
        if (b.any()) b.trigger(); else alert('CSV export unavailable.');
    });
