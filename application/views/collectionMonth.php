...
<head>
    ...
    <title>Collection Summary Per Month</title>
...
<body>
    ...
    <p>For the Month of <?= $selected_month; ?></p>
                    <!-- Print Button for Payment Report -->
                    <!-- <button class="btn btn-secondary float-right no-print" onclick="window.print()">Print Per Month Summary</button> -->
    ...
    <tbody>
        <?php if (!empty($collection_data)): ?>
            <?php 
            $month_total = 0; // Initialize total for the month
            foreach ($collection_data as $row): 
                $amount = floatval(str_replace(',', '', $row->Amount)); // Ensure proper formatting
                $month_total += $amount; // Add amount to total
            ?>
                <tr>
                    <td><?= date('Y-m-d', strtotime($row->PDate)); ?></td>
                    <td><?= $row->ORNumber; ?></td>
            ...
            <tr>
                ...
                <td style="text-align:right; font-weight:bold;"><?= number_format($month_total, 2); ?></td>
                <td colspan="2"></td>
            </tr>
        <?php else: ?>
            <tr>
                <td colspan="7" style="text-align:center;">No data available for the selected month.</td>
            </tr>
        <?php endif; ?>
    </tbody>
</table>
