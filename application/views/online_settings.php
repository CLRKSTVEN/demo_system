...
<div id="wrapper">
  ...
  <style>
    .settings-card { border:0; box-shadow:0 8px 28px rgba(0,0,0,.08); border-radius:18px; overflow:hidden; }
    .settings-head {
    ...
    .switch-xl { transform:scale(1.35); transform-origin:left center; }
    .muted { color:#6c757d; }
    .hint { font-size:.92rem; color:#6c757d; margin-top:10px; }
    ...
    .pending-dot { width:8px; height:8px; border-radius:50%; background:#ffc107; display:inline-block; margin-right:6px; }
    .btn-link-muted { color:#6c757d; }
    .btn-link-muted:hover { color:#495057; text-decoration:none; }
    .scope-badges .badge { font-weight:600; letter-spacing:.2px; }
    .callout {
      border-left:4px solid #3b82f6; background:#f8fafc; padding:12px 14px; border-radius:10px;
    }
    .callout .title { font-weight:700; margin-bottom:6px; }
    .mini { font-size:.9rem; }
    .cardish {
      border:1px solid #eef0f3; border-radius:12px; padding:12px 14px;
      display:flex; gap:12px; align-items:flex-start; background:#fff;
    }
    .cardish .ic {
      width:34px; height:34px; border-radius:10px; background:#eef6ff;
    ...
    .ct-toast {
      ...
      min-width: 280px; max-width: 360px; padding: 12px 14px 12px 12px;
      display: flex; align-items: flex-start; gap: 10px;
      border-left: 5px solid #22c55e;
      opacity: 0; transform: translateY(-10px); pointer-events: none;
      transition: opacity .2s ease, transform .2s ease;
    }
    .ct-toast.is-visible { opacity: 1; transform: translateY(0); pointer-events: auto; }
    .ct-toast__icon { font-size: 20px; line-height: 1; margin-top: 2px; color: #22c55e; }
    .ct-toast__body { flex: 1; }
    .ct-toast__title { font-weight: 700; margin: 0 0 2px 0; font-size: 0.95rem; }
    .ct-toast__text { margin: 0; font-size: 0.92rem; color: #475569; }
    .ct-toast__close {
      appearance: none; border: 0; background: transparent; color: #6b7280;
      font-size: 18px; line-height: 1; padding: 0 4px; cursor: pointer;
    }
    .ct-toast__close:hover { color: #111827; }
  </style>
          ...
          <div class="col-lg-8 col-md-10">
            <div class="card settings-card">
              <div class="settings-head">
                ...
                <div>
                  <h4 class="mb-0">Online Settings</h4>
                  <small style="opacity:.9">
                    Control the visibility of Online Payments across <b>Accounting</b> and the <b>Student Portal</b>.
                  </small>
                  <div class="scope-badges mt-2">
                    <span class="badge badge-light text-dark">Scope:</span>
                    <span class="badge badge-info">Accounting</span>
                    <span class="badge badge-success">Student Portal</span>
                  </div>
                </div>
              </div>
              ...
              <div class="settings-body">
                ...
                <div class="d-flex align-items-center justify-content-between mb-2">
                  <div class="muted">
                    ...
                    <?php if ($lastUpdated): ?>
                      <small class="d-block mt-1">Last updated: <?= htmlspecialchars($lastUpdated, ENT_QUOTES, 'UTF-8'); ?></small>
                    <?php endif; ?>
                  </div>
                  <div class="d-flex align-items-center" id="statusWrap">
                    <?php if ($isOn): ?>
                      <span id="statusBadge" class="badge badge-success badge-pill">ENABLED</span>
                    <?php else: ?>
                      <span id="statusBadge" class="badge badge-danger badge-pill">DISABLED</span>
                    <?php endif; ?>
                    <small id="pendingNote" class="ml-2 text-warning d-none">
                      <span class="pending-dot"></span>Not saved yet
                    </small>
                  </div>
                </div>
                ...
                <!-- Toggle -->
                <form method="post" action="<?= base_url('OnlineSettings/save'); ?>" id="toggleForm" novalidate>
                  <div class="d-flex align-items-center mb-2">
                    <div class="form-check form-switch">
                      <input class="form-check-input switch-xl" type="checkbox" id="showOP"
                             name="show_online_payments" value="1" <?= $isOn ? 'checked' : '' ?>>
                    </div>
                    <label for="showOP" class="mb-0 ml-3">
                      <strong>Show “Online Payments” (global)</strong>
                      <div class="muted mini">A single switch that affects Accounting menus and the Student “Pay Now via Dragonpay” button.</div>
                    </label>
                  </div>

                  <div class="hint">
                    <div class="cardish mb-2">
                      <div class="ic"><i class="ion ion-md-business"></i></div>
                      <div>
                        <div class="title">What this controls (Accounting)</div>
                        <ul class="mb-0 list-compact soft">
                          <li><b>Payment Verification</b></li>
                          <li><b>Online Payments (All)</b></li>
                          <li><b>Online Payments (Denied)</b></li>
                        </ul>
                      </div>
                    </div>

                    <div class="cardish">
                      <div class="ic"><i class="ion ion-md-school"></i></div>
                      <div>
                        <div class="title">What this controls (Student Portal)</div>
                        <ul class="mb-0 list-compact soft">
                          <li><b>“Pay Now via Dragonpay”</b> button on the Student’s Account Tracking page</li>
                          <li>“Pay” buttons per term/row when there is a balance</li>
                        </ul>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="d-flex align-items-center flex-wrap gap-2">
                    <button type="submit" class="btn btn-primary" id="saveBtn" disabled>
                      <i class="ion ion-md-save"></i> Save changes
                    </button>
                    <button type="button" class="btn btn-link btn-link-muted ml-2" id="resetBtn" disabled>
                    </button>

                    <div class="ml-auto soft mini">
                      <i class="ion ion-md-flash"></i> Effective immediately •
                    </div>
                  </div>
                </form>

                <?php if (!$isOn): ?>
                  <div class="mt-3 soft mini">
                    <i class="ion ion-md-information-circle-outline"></i>
                    When disabled, students will not see the Pay now via Dragonpay , and Accounting Online payment is hidden.
                  </div>
                <?php endif; ?>
